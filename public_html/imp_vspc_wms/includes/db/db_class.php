<?php
	...
	//===========================================================================================//
				...
				foreach($temp_obj['fields'] as $k => $v){
					...
					if($k != $temp_obj["primary_field"]){
						...
						if(trim($temp_obj['fields'][$k]["input_value"]) == ""){
							$values[]	= "''";
							//$values[] = "NULL";  //Changed to "" for the sake of MySQL  ... T.O.
 						}else{
		...
		function record_delete($table_name, $temp_obj, $pk){
			...
			$sql = "DELETE FROM ".$table_name." WHERE ".$temp_obj["primary_field"]." = '".$pk."'";
	...
	//===========================================================================================//
		...
		function MySQLquery($sqlquery){
			...
			//MySQL
			$this->result = $this->link->query($sqlquery);
			//loop trough results, create results arr to return
			$result_arr="";
			$temp_arr="";

			if($this->result){
				while($temp_arr= $this->result->fetch_assoc()){
					if($temp_arr){
						$result_arr[]=$temp_arr;
						$temp_arr="";
					}
				...
				print_r("Error with Query<br/>");
				print_r($sqlquery);
				print_r("<br/>", $this->link->error);
		...
		function MySQLescapeString($string_to_escape) {
			$replaced_string =  $this->link->real_escape_string($string_to_escape);
