...
{
	function db(){
		$this->server 	= "MSSQL"; 		// connect to database in MSSQL or MySQL
		
		//login vars
		if($this->server == "MySQL") {
			//if database is in MySQL
			$this->database	= "viaspace_db";
			$this->host		= "localhost";
			$this->username	= "root";
			$this->password	= "";
...
//===========================================================================================//
	...
	function MySQLquery($sqlquery){
		//MySQL
		$this->result = mysql_db_query ($this->database, $sqlquery, $this->link);
		//loop trough results, create results arr to return
		$result_arr="";
		$temp_arr="";
		while($temp_arr=mysql_fetch_assoc($this->result )){
			if($temp_arr){
				$result_arr[]=$temp_arr;
				$temp_arr="";
			}
	...
	function MySQLescapeString($string_to_escape) {
		$replaced_string =  mysql_escape_string($string_to_escape);
