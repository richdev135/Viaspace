...
//define vars
	...
	$strQuery = $strQuery . " AND  vspc_language_id = '".$lang_id."'";
	if (!trim($sort) == "" ){
...
<tr>
<td align="left">
<a class = "add_link" href="object_add.php?obj=<?php echo $obj; ?>">Add <?php echo $MyObjects[$obj]["caption"]; ?></a>
</td>
<td align="right">
<?php
	...
	if (!trim($sort_d) == ""){
 	...
 	if (!($all == "true")){ 
		...
		if (count($res) > $num_per_page){ 
			echo "<a href=\"".$_SERVER['SCRIPT_NAME']."?"."obj=".$obj."&all=true".$extra_vars."\">All</a>\n";
		}
		if ($prev_num >= 0 ){ 
			echo "<a href=\"".$_SERVER['SCRIPT_NAME']."?"."obj=".$obj."&start=".$prev_num.$extra_vars."\">&lt;&lt;</a>\n"; 
		}
		...
		if (count($res) > $next_num){
			echo "<a href=\"".$_SERVER['SCRIPT_NAME']."?"."obj=".$obj."&start=".$next_num.$extra_vars."\">&gt;&gt;</a>\n"; 
		}
	}else{
		echo "1 - ".count($res);
		echo " <a href=\"".$_SERVER['SCRIPT_NAME']."?"."obj=".$obj."&all=false".$extra_vars."\">First ".$num_per_page."</a>\n";
	} 
?>
...
<?php
	...
	foreach ($MyObjects[$obj]["fields"] as $x => $v ){
		...
		if ($v["display"] and (!(strtolower($v["input_type"]) == strtolower("wysiwyg")) ) ){
			...
			?>
			<a href="<?php 
						echo $_SERVER['SCRIPT_NAME']."?obj=".$obj."&sort=".$x; 
						if(trim($sort)==trim($x)){
...
if(is_array($res)){
	foreach ($res as $x => $v){
		if ((intval($x) >= intval($current_start_num)) && (intval($x) < intval(($current_start_num + $num_per_page))) || ($all=="true") ) {
						...
						<td><?php
							if( strtolower($v2["input_type"]) == strtolower("order") ){
								...
								if(trim($sort) == trim($y)) {
									//display order links
									if(isset($res[($x-1)][$y])){
										echo "<a href=\"object_change_order.php?";
										echo "obj=".$obj."&";
										echo "sort=".$sort."&";
										echo "value=".$res[$x][$y]."&";
										echo "move=up\" ";
			...
			<td>
			<?php
			$strQuery2 = "Select * from ". $obj .", vspc_languages where vspc_translate_id = '".$res[$x]["vspc_translate_id"]."' ";
			$strQuery2 = $strQuery2 . " AND vspc_languages.vspc_lang_id = ".$obj.".vspc_language_id ";
			$strQuery2 = $strQuery2 . " AND ".$obj.".".$MyObjects[$obj]["primary_field"]." > 0 ";
			$strQuery2 = $strQuery2 . " ORDER BY vspc_languages.vspc_lang_name";
			$res2 = $objDb->DBquery($strQuery2);
				
				foreach($res2 as $k2 => $v2){
					echo "<a href=\"object_edit.php?obj=".$obj."&pk=".$v2["vspc_translate_id"]."&lang_id=".$v2["vspc_language_id"]."\">".$v2["vspc_lang_name"] ."</a><BR>\n";
				}
