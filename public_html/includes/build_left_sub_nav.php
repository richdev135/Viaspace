<?php
	...
	//get parents children for left side nav
		$objParent1 = new page();
		$objParent1->translate_id = $objPage->parent_id;	//parent id
		$objParent1->language_id = $lang_id;
		$objParent1->get_page($objDb);

		$objParent = new page();
		$objParent->translate_id = $objParent1->parent_id;	//parent id
		...
		foreach($objParent->children as $k => $v){
			...
			if($objChild->get_page($objDb)){
				...
				if(strlen($objChild->url)> 0){
					...
					print($page_url."?id=".$objChild->page_id);
				}
				print("\"><img src=\"images/menuarrow2.gif\"> ");
				print($objChild->title);
				print("</a>");
				
				if($objPage->parent_id == $objChild->page_id){
					foreach($objParent1->children as $k2 => $v2){
						$sub_child_id = $v2;
